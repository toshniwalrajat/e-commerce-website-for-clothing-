...
export default async function WomenSubPage({ params }: { params: Promise<{ sub: string }> }) {
  const { sub } = await params; // Next 15: params is a Promise
  const list = fetchByPath(['women', sub]);

  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      <h1 className="mb-4 font-medium capitalize">Women / {sub}</h1>
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {list.map((p) => (
          <article key={p.id} className="rounded-xl border border-neutral-200 bg-white p-3">
            ...
            <Link href={`/product/${p.id}`} className="mt-2 inline-block text-sm text-neutral-700 hover:text-black">
              View →
            </Link>
          </article>
        ))}
