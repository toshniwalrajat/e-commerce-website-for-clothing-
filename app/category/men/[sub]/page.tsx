...
export default async function MenSubPage({ params }: { params: Promise<{ sub: string }> }) {
  const { sub } = await params; // Next 15: unwrap server params
  const list = fetchByPath(['men', sub]);

  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      <h1 className="mb-4 font-medium capitalize">Men / {sub}</h1>
